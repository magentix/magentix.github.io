...
Ce **replace** est inspiré de celui proposé par **yireo** : [yireo/magento2-replace-all](https://github.com/yireo/magento2-replace-all/blob/magento-2.4.2/composer.json). Je l'ai enrichi avec d'autres modules non indispensables pour une installation basique (two-factor-auth, login-as-customer, re-captcha...).

Il a été testé avec succès sur **Magento Open-Source 2.4.3**. À adapter selon vos besoins (on se rapproche ici des mêmes features de base d'un Magento 1.9).
